...
try
{
	...
	{
		//Get record count
		$result = mysql_query("SELECT COUNT(*) AS RecordCount FROM Hundebesitzer WHERE PersonID = " . $_GET["PersonID"] . ";");
		$row = mysql_fetch_array($result);
		$recordCount = $row['RecordCount'];

		//Get records from database
		$result = mysql_query("SELECT  Hunde.HundID, Hundebesitzer.PersonID, Rufname, Zwingername, Rassen.Name, Geschlecht, Wurftag, Farbe FROM `Hundebesitzer`  INNER JOIN Hunde ON Hunde.HundID=Hundebesitzer.HundID INNER JOIN Rassen ON Rassen.RasseID=Hunde.RasseID WHERE Hundebesitzer.PersonID = " . $_GET["PersonID"] . ";");
